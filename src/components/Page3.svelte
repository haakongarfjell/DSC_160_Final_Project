<script>
    ...
    svg.append("text")
        ...
        .text("Cruise Control by Using P controller");
    ...
    // Append the vertical black solid line for steady state error at 90 sec
   // Find the index corresponding to 90 seconds
    const index90Seconds = simulationData[1].findIndex(time => time >= 90);
    ...
    .attr('x2', x(90))
    .attr('y1', y(setpoint_velocity))
    .attr('y2', y(simulationData[0][index90Seconds]))
    .attr('stroke', 'black')
    .attr('stroke-width', 2);

    // Append text label for steady state error
    svg.append("text")
        .attr("x", x(90) - 120)
        .attr("y", (y(setpoint_velocity) + y(simulationData[0][index90Seconds])) / 2)
    ...
    // Function to update simulation data with new Kp value
...
<!-- Slider for adjusting Kp parameter -->
<div style="text-align: center; margin-top: 20px;">
    <label for="Kp">Kp:</label>
    <input type="range" id="Kp" name="Kp" min="0" max="100" step="1" bind:value={Kp} on:input={updateKp}>
    <span>{Kp}</span>
</div>
